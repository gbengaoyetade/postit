...
describe('Message ', () => {
  ...
  it('should create message when inputs are valid', (done) => {
    ...
    supertest(app).post('/api/group/3/message')
    .set('x-access-token', firstUserToken).send(message)
    .end((err, res) => {
...
describe('Get Messages', () => {
  ...
  (done) => {
    ...
    supertest(app).post('/api/group/3/message')
    .set('x-access-token', secondUserToken).send(message)
    .end((err, res) => {
      assert.equal(res.statusCode, 201);
      assert.equal(res.body.message.messageBody, message.messageBody);
      assert.equal(res.body.message.messagePriority, message.messagePriority);
  ...
  (done) => {
    ...
    supertest(app).get('/api/group/3/messages').set('x-access-token', secondUserToken)
    .send()
    .end((err, res) => {
  ...
  (done) => {
    ...
    supertest(app).post('/api/group/3/message').set('x-access-token', firstUserToken)
    .send(message)
    .end((err, res) => {
      assert.equal(res.statusCode, 500);
      assert.equal(res.body.error, 'Internal server error');
